...
**NOTE: (1) iOS does not have a toggle button, so this should be expalined here. (2) The toggle button image shows a UISegmentedControl, not the cards example's toggle button - which is different. (3) The card's example should go in the cards page - it doesn't belong here.**
